<template>
    <section class="flex flex-col justify-center items-center h-screen gap-8 shadow-inner">
        <div id="cont_form" class="flex flex-col justify-center items-center gap-10 bg-[whitesmoke] p-20 rounded-md">
            <h2 class="text-4xl font-bold tracking-wider drop-shadow-md">REGISTER</h2>
            <form @submit.prevent="registerUser" class="flex flex-col gap-5">
                <div class="flex">
                    <label class="shadow-md p-2 px-3 bg-green-500 text-white" for="email"><i class="fa-solid fa-envelope"></i></label>
                    <input class="shadow-md outline-none rounded-r-md ps-2" type="email" id="email" v-model="email" required placeholder="Correo Electrónico">
                </div>
                <div class="flex">
                    <label class="shadow-md p-2 px-3 bg-green-500 text-white" for="password"><i class="fa-solid fa-lock"></i></label>
                    <input class="shadow-md outline-none rounded-r-md ps-2" type="password" id="password" v-model="password" required placeholder="Contraseña">
                </div>
                <button type="submit" class=" hover:scale-105 transition p-2 mt-4 bg-green-600 shadow-lg text-white rounded-md font-bold">Registrarse</button>
            </form>
            <p>Ya tienes cuenta? <router-link to="/login" class="text-green-700 underline">Inicia Sesion</router-link></p>
        </div>
    </section>
</template>

<script>
import { auth } from '@/db/firebase'
import { createUserWithEmailAndPassword } from 'firebase/auth'
export default {
    name: 'RegisterView',
    data() {
        return {
            email: '',
            password: ''
        }
    },
    methods:{
        async registerUser(){
            try {
                await createUserWithEmailAndPassword(auth, this.email, this.password)
                console.log('Usuario creado:')
                this.$router.push({name: 'perfil'})
            }catch (error){
                console.log(error)
                alert(error.message)
            }
        }
    }
}
</script>

<style lang="sass" scoped>
section
    background: #52c234  /* fallback for old browsers */
    background: -webkit-linear-gradient(to top, #061700, #52c234)  /* Chrome 10-25, Safari 5.1-6 */
    background: linear-gradient(to top, #061700, #52c234) /* W3C, IE 10+/ Edge, Firefox 16+, Chrome 26+, Opera 12+, Safari 7+ */
</style>